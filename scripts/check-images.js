...
console.log(`${colors.cyan}📸 Image Replacement Status Checker${colors.reset}\n`);
...
if (checkDirectory(path.join(assetsDir, 'about'))) {
  expectedImages.about.forEach(imageName => {
    ...
    if (result.exists) {
      ...
      console.log(`  ${colors.red}❌${colors.reset} ${imageName} ${colors.yellow}(MISSING)${colors.reset}`);
    }
  });
} else {
  console.log(`  ${colors.red}❌ Directory doesn't exist: about/${colors.reset}`);
...
if (checkDirectory(path.join(assetsDir, 'homepage'))) {
  ...
  console.log(`  ${colors.red}❌ Directory doesn't exist: homepage/${colors.reset}`);
...
if (checkDirectory(path.join(assetsDir, 'contact'))) {
  expectedImages.contact.forEach(imageName => {
    ...
    if (result.exists) {
      ...
      console.log(`  ${colors.yellow}⚠️${colors.reset}  ${imageName} ${colors.yellow}(OPTIONAL - MISSING)${colors.reset}`);
    }
  });
} else {
  console.log(`  ${colors.yellow}⚠️  Directory doesn't exist: contact/ (optional)${colors.reset}`);
...
Object.keys(expectedImages.services).forEach(serviceDir => {
  ...
  if (checkDirectory(servicePath)) {
    serviceImages.forEach(imageName => {
      ...
      if (result.exists) {
        totalFound++;
        console.log(`  ${colors.green}✅${colors.reset} services/${serviceDir}/${imageName}`);
      } else {
        console.log(`  ${colors.red}❌${colors.reset} services/${serviceDir}/${imageName} ${colors.yellow}(MISSING)${colors.reset}`);
      }
    });
  } else {
    console.log(`  ${colors.yellow}⚠️  services/${serviceDir}/ ${colors.yellow}(Directory doesn't exist)${colors.reset}`);
...
console.log(`  Images missing: ${colors.red}${totalExpected - totalFound}${colors.reset}`);
console.log(`  Completion: ${colors.cyan}${Math.round((totalFound / totalExpected) * 100)}%${colors.reset}`);
console.log(`${colors.cyan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}\n`);

// Recommendations
if (totalFound === 0) {
  console.log(`${colors.yellow}💡 Recommendation:${colors.reset}`);
  console.log(`  Start with the About page 'our-story.jpg' - it's the highest priority!\n`);
  console.log(`  See IMAGE_REPLACEMENT_GUIDE.md for detailed instructions.\n`);
} else if (totalFound < totalExpected) {
  console.log(`${colors.yellow}💡 Next Steps:${colors.reset}`);
  console.log(`  Continue adding images following the priority order in IMAGE_REPLACEMENT_GUIDE.md\n`);
} else {
  console.log(`${colors.green}🎉 All images found!${colors.reset}`);
  console.log(`  Don't forget to uncomment the imports and Image components in your .astro files!\n`);
}
